...
figure(2); clf;
for i = 1:nclick
  patch1 = Itrain(8*blocky(i)+(-63:64),8*blockx(i)+(-63:64));
  figure(2); subplot(3,2,i); imshow(patch1);
  
  patch2 = Itrain1(8*blocky1(i)+(-63:64),8*blockx1(i)+(-63:64));
  figure(2); subplot(3,2,i+1); imshow(patch2);
end

figure(2); subplot(3,2,i+2); imshow(0.5*(patch1+patch2));
...
Itest= im2double(rgb2gray(imread('../data/internet2.jpg')));
